...
export default async function Page({ params, searchParams }: Props) {
  const model = await load((await params).routeSlug);
  const filterValues = parseRouteFilterQuery(await searchParams, ROUTE_PAGE_FILTER_FIELDS);
  const filters = serializeRouteSearchFilters(filterValues, ROUTE_PAGE_FILTER_FIELDS);
  const routes = await searchRoutes(
    { type: "city_pair", from: model.route.origin.slug, to: model.route.destination.slug },
    filters,
    filterValues.after ?? null,
  );
  return <RoutePageScreen model={model} routes={routes} filterValues={filterValues} />;
}

...
export function RoutePageScreen({
  ...
  return (
    <main className="pseo-page route-page">
      <div className="pseo-container">
        ...
        {model.summary && (
          <StatGrid
            items={[
              { label: "Nonstop options", value: model.summary.directOptions },
              {
                label: "Connecting options",
                value: model.summary.indirectOptions,
              },
              {
                label: "Fastest nonstop",
                value:
                  model.summary.fastestDirectMinutes === null
                    ? "Unknown"
                    : formatDuration(model.summary.fastestDirectMinutes),
              },
              {
                label: "Fastest connection",
                value:
                  model.summary.fastestIndirectMinutes === null
                    ? "Unknown"
                    : formatDuration(model.summary.fastestIndirectMinutes),
              },
            ]}
          />
        )}
        <section className="pseo-section observed-prices">
          <h2>Recent price observations</h2>
          {model.observedPrices.length ? (
            <>
              <div>
                {model.observedPrices.slice(0, 3).map((price) => (
                  <ObservedPriceCard key={price.reference} price={price} />
                ))}
              </div>
              <p>{model.affiliateDisclosure}</p>
            </>
          ) : (
            <p>
              No recent price observation is available. Check again after the
        ...
        {model.sections.length ? (
          <section className="pseo-section">
            <h2>
              Plan your {origin.name}–{destination.name} journey
            </h2>
            <div className="route-facts">
              ...
              {model.facts.map((fact) => (
                <article key={fact.type}>
                  <h3>{fact.title}</h3>
                  <p>{fact.body}</p>
                  {fact.sourceUrl ? (
                    <a href={fact.sourceUrl} rel="noreferrer">
                      Official source
                    </a>
                  ) : null}
                </article>
        ...
        {model.affiliateOffers?.length ? (
          <section className="pseo-section">
            <h2>Sponsored Travel Services</h2>
            <p>{model.affiliateDisclosure}</p>
            {model.affiliateOffers.map((offer) => (
              <a key={offer.href} href={offer.href} rel="sponsored nofollow">
                {offer.title}
              </a>
            ))}
          </section>

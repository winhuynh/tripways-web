...
export function RoutePageScreen({ model }: { model: RoutePageModel }) {
  ...
  return <main className="pseo-page route-page"><div className="pseo-container">
    ...
    <section className="pseo-section observed-prices"><h2>Recent price observations</h2>
      {model.observedPrices.length?<><div>{model.observedPrices.slice(0,3).map(price=><ObservedPriceCard key={price.reference} price={price}/>)}</div><p>{model.affiliateDisclosure}</p></>:<p>No recent price observation is available. Check again after the next provider refresh.</p>}
    </section>
